...
class AES(object):
    ...
    def encrypt(self, msg):
        """
        AES encryption of arbitrary length message

        :param msg: arbitrary length message
        :return: ciphertext
        """
        # PKCS7 padding
        pad_length = 16 - len(msg)%16
        msg += bytes([pad_length]) * pad_length
        cipher = b""
        for i in range(0, len(msg), 16):
            cipher += self.encrypt_block(msg[i:i+16])
        return cipher

    def decrypt(self, cipher):
        """
        AES decryption of arbitrary length ciphertext
        ...
        for i in range(0, len(cipher), 16):
            msg += self.decrypt_block(cipher[i:i+16])
        pad_length = msg[-1]
        msg = msg[:-pad_length]
        assert self.encrypt(msg) == cipher
        return msg
...
def test1():
    ...
    cipher = aes.encrypt_block(b"fedcba9876543210")
    decrypted = aes.decrypt_block(cipher)
    print(f"encryption: {cipher}\ndecryption: {decrypted}")

def test2():
    aes = AES(b"0123456789abcdef")
    cipher = aes.encrypt(b"1")
